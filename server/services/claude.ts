...
/** Shared tool schema for community report generation — used by both neighborhood and block-level reports. */
const REPORT_TOOL_SCHEMA: Anthropic.Messages.Tool['input_schema'] = {
  type: 'object' as const,
  properties: {
    neighborhoodName: { type: 'string', description: 'Name of the neighborhood or anchor location' },
    language: { type: 'string', description: 'Language the report is written in' },
    summary: { type: 'string', description: 'A 2-sentence welcome greeting' },
    goodNews: {
      type: 'array',
      items: { type: 'string' },
