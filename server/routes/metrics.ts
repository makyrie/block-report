...
import { fetchCommunityMetrics, buildCommunityMetricsResponse } from '../services/metrics.js';
...
router.get('/', async (req, res) => {
  ...
  try {
    const metrics = await fetchCommunityMetrics(cleaned);
    res.json(buildCommunityMetricsResponse(metrics));
  } catch (err) {
